...
export class UserService {
   ...
   */
  async registerUser(userData: ICreateUser): Promise<{ user: IUser; token: string }> {
   ...
   */
  async authenticateUser(credentials: IUserAuth): Promise<{ user: IUser; token: string }> {
    ...
    const userRepo = this.userRepository as InMemoryUserRepository;
   ...
   */
  async updateUser(id: string, updateData: IUpdateUser): Promise<IUser | null> {
    if (!id) {
      throw new Error('ID de usuario requerido');
    }
